...
<body>
  ...
  <table style="width:90%;" class="table table-hover" id="List">
    ...
    <tbody>
      <!-- 공지글 -->
	   <c:forEach var="vo" items="${shareboardNotice }">
	   	<tr>
	    	<td>${vo.shareboard_bno }</td>
	    	<td>
	    		<a style="color: black; 
	          text-decoration: none;
	          text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3);"
	           href="detail?bno=${vo.shareboard_bno }&page=${pageMaker.criteria.page}">
	    			<span style="color:red;"><c:out value="${vo.shareboard_title }"/> 
	    				<c:if test="${vo.shareboard_replycount ne '0' }"> (${vo.shareboard_replycount })</c:if>
	    			</span>
    			</a>
    		</td>
	    	<td>${vo.userid }</td>
	    	<fmt:formatDate value="${vo.shareboard_date }" pattern="yyyy-MM-dd HH:mm:ss" var="shareboard_date"/>
	    	<td>${shareboard_date }</td>
	   	</tr>
	   </c:forEach>
		
		<!-- 게시글 -->	   
	     <c:forEach var="vo" items="${shareboardList }">
	       <tr>
	         <td>${vo.shareboard_bno }</td>
	         <td>
	         	<a style="color: black; 
	          ...
	          text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3);"
	           href="detail?bno=${vo.shareboard_bno }&page=${pageMaker.criteria.page}">
	         		<c:out value="${vo.shareboard_title }"/> 
	         		<c:if test="${vo.shareboard_replycount ne '0' }"><span style="color:red;"> (${vo.shareboard_replycount })</span></c:if>
	         	</a>
         	 </td>
	         <td>${vo.userid }</td>
	         <fmt:formatDate value="${vo.shareboard_date }" pattern="yyyy-MM-dd HH:mm:ss" var="shareboard_date"/>
