...
<title>${shareboardVO.shareboard_title }</title>
<style type="text/css">
...
<body>
	<h6><a href="list"style="color: black; 
	          text-decoration: none;"><strong>공유게시판</strong></a> > ${shareboardVO.shareboard_bno }</h6>
	<hr>
	<!-- 본문 -->
	<H2><c:out value="${shareboardVO.shareboard_title }"/></H2>
	<fmt:formatDate value="${shareboardVO.shareboard_date }" pattern="yyyy-MM-dd HH:mm:ss" var="shareboard_date" />
		작성자 : ${shareboardVO.userid } | 작성일 : ${shareboard_date } | 	
		<!-- 공지글 설정 (관리자 전용) -->
      	<c:if test="${part eq 'manager' }"> 
       	  <select id="notice">
		  	<option value="0" <c:if test="${shareboardVO.shareboard_notice eq '0' }">selected</c:if>>공지_0</option>
		  	<option value="1" <c:if test="${shareboardVO.shareboard_notice eq '1' }">selected</c:if>>공지_1</option>
		  	<option value="2" <c:if test="${shareboardVO.shareboard_notice eq '2' }">selected</c:if>>공지_2</option>
		  	<option value="3" <c:if test="${shareboardVO.shareboard_notice eq '3' }">selected</c:if>>공지_3</option>
		  </select>
	...
	<table style="width:90%;">
		<tr>
			<td>
			    <div>
			    	<br>
						${shareboardVO.shareboard_content }
			    	<br>
			    </div>
			</td>
		</tr>
	...
	<div>
		...
		<c:if test="${shareboardVO.userid eq loginId and blacklist ne 1}">
			<a href="update?bno=${shareboardVO.shareboard_bno }&page=${page}"><input type="button" class="btn btn-light" value="글 수정"></a>
			<a href="delete?bno=${shareboardVO.shareboard_bno }" class="deleteLink" onclick="return false"><input type="button" class="btn btn-light" onclick="deleteConfirm()" value="글 삭제"></a>
		</c:if>
	</div>
	<br>
	...
	<input type="hidden" id="bno" value="${shareboardVO.shareboard_bno }">
	...
	$(document).ready(function() {
		...
		// ======================================================================================
			...
			function bookmarkCheck(){
				...
				var bno = $('#bno').val();
 				var bookmark = {
					userid : id,
					bookmark_table_name : "shareboard_table",
					bookmark_bno : bno
				};
  	...
  	// ******************************************************************
		...
		// WebSocket
	  	...
	  	var sock = new WebSocket('ws://' + window.location.host + '/prj/shareboard/websocket');
