...
<body>
  <h6><a href="list" style="color: black; 
	          text-decoration: none;"><strong>질문게시판</strong></a> > <a href="detail?bno=${qaboardVO.qaboard_bno }&page=${page}" style="color: black; 
	          text-decoration: none;">>${qaboardVO.qaboard_bno }</a> > 수정</h6>
  <hr>
  <form action="update" method="POST">
	<div>
  	<input type="hidden" name="page" value="${page }">
    <input type="hidden" name="userid" value="${loginId }">
    <input type="hidden" name="qaboard_bno" value="${qaboardVO.qaboard_bno }">
      <fmt:formatDate value="${qaboardVO.qaboard_date }" pattern="yyyy-MM-dd HH:mm:ss" var="qaboard_date" />
      <p>작성자 : ${qaboardVO.userid } | 작성일 : ${qaboard_date } | 글 번호 : ${qaboardVO.qaboard_bno }</p>
      <p>제목 : 
      	<input style="width:50%;" type="text" name="qaboard_title" placeholder="제목 입력" required="required" value="${qaboardVO.qaboard_title }" >
      	<!-- 공지글 설정 (관리자 전용) -->
      	<c:if test="${part eq 'manager' }"> | 
       	  <select name="qaboard_notice" id="notice">
	...
	<!-- TinyMCE -->
    <div>
		<textarea id="editor" name="qaboard_content" required>${qaboardVO.qaboard_content }</textarea>
    </div>
    <!-- 하단버튼 -->
    <div>
