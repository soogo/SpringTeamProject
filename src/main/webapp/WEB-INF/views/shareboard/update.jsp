...
<body>
  <h6><a href="list" style="color: black; 
	          text-decoration: none;"><strong>공유게시판</strong></a> > <a href="detail?bno=${shareboardVO.shareboard_bno }&page=${page}" style="color: black; 
	          text-decoration: none;">>${shareboardVO.shareboard_bno }</a> > 수정</h6>
  <hr>
  <form action="update" method="POST">
	<div>
  	<input type="hidden" name="page" value="${page }">
    <input type="hidden" name="userid" value="${loginId }">
    <input type="hidden" name="shareboard_bno" value="${shareboardVO.shareboard_bno }">
      <fmt:formatDate value="${shareboardVO.shareboard_date }" pattern="yyyy-MM-dd HH:mm:ss" var="shareboard_date" />
      <p>작성자 : ${shareboardVO.userid } | 작성일 : ${shareboard_date } | 글 번호 : ${shareboardVO.shareboard_bno }</p>
      <p>제목 : 
      	<input style="width:50%;" type="text" name="shareboard_title" placeholder="제목 입력" required="required" value="${shareboardVO.shareboard_title }" >
      	<!-- 공지글 설정 (관리자 전용) -->
      	<c:if test="${part eq 'manager' }"> | 
       	  <select name="shareboard_notice" id="notice">
	...
	<!-- TinyMCE -->
    <div>
		<textarea id="editor" name="shareboard_content" required>${shareboardVO.shareboard_content }</textarea>
    </div>
    <!-- 하단버튼 -->
    <div>
