...
<body>
  ...
  <c:if test="${loginId ne null and blacklist ne 1}">
  	<p><a href="register"><input class="btn btn-light" type="button" value="글작성"></a></p>
  </c:if>
  <br>
  <!-- ********************** Board Table ********************** -->
  <table style="width:90%;" class="table table-hover" id="List">
    ...
    <tbody>
      <!-- 공지글 -->
	   <c:forEach var="vo" items="${freeboardNotice }">
	   	<tr>
	    	<td>${vo.freeboard_bno }</td>
	    	<td>
	    		<a style="color: black; 
	          text-decoration: none;
	          text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3);"
	           href="detail?bno=${vo.freeboard_bno }&page=${pageMaker.criteria.page}">
	    			<span style="color:red;"><c:out value="${vo.freeboard_title }"/> 
	    				<c:if test="${vo.freeboard_replycount ne '0' }"> (${vo.freeboard_replycount })</c:if>
	    			</span>
    			</a>
    		</td>
	    	<td>${vo.userid }</td>
	    	<fmt:formatDate value="${vo.freeboard_date }" pattern="yyyy-MM-dd HH:mm:ss" var="freeboard_date"/>
	    	<td>${freeboard_date }</td>
	   	</tr>
	   </c:forEach>
	     ...
	     <c:forEach var="vo" items="${freeboardList }">
	       <tr>
	         ...
	         <td>
	         	<a style="color: black; 
	          text-decoration: none;
	          text-shadow: 0 1px 1px rgba(0, 0, 0, 0.3);" href="detail?bno=${vo.freeboard_bno }&page=${pageMaker.criteria.page}">
	         		<c:out value="${vo.freeboard_title }"/> 
	         		<c:if test="${vo.freeboard_replycount ne '0' }"><span style="color:red;"> (${vo.freeboard_replycount })</span></c:if>
	         	</a>
         	 </td>
	         <td>${vo.userid }</td>
	         <fmt:formatDate value="${vo.freeboard_date }" pattern="yyyy-MM-dd HH:mm:ss" var="freeboard_date"/>
